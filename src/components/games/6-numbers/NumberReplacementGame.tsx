...
export function NumberReplacementGame({ theme, onComplete, allowSkip = true }: { theme: NumberReplacementTheme; onComplete?: (score?: number, maxScore?: number) => void; allowSkip?: boolean }) {
  ...
  return <div className={`w-full max-w-3xl mx-auto rounded-[2rem] border-8 ${theme.colors.border} ${theme.colors.panel} p-5 md:p-8 shadow-xl`}>
    ...
    {isCompleted ? <div className="rounded-3xl bg-white p-6 text-center shadow-inner"><h3 className="mb-5 text-3xl font-black text-emerald-700">Game complete!</h3>{allowSkip === false && onComplete && <Button size="lg" onClick={() => onComplete?.(score, attempts)}>Continue to Next Game</Button>}<Button className="ml-3" variant="outline" onClick={resetGame}>Play Again</Button></div> : <div className="rounded-3xl bg-white p-6 text-center shadow-inner">
