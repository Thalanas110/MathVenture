...
export function NumberReplacementGame({ theme, onComplete }: { theme: NumberReplacementTheme; onComplete?: () => void }) {
  ...
  return <div className={`w-full max-w-3xl mx-auto rounded-[2rem] border-8 ${theme.colors.border} ${theme.colors.panel} p-5 md:p-8 shadow-xl`}>
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-2"><div className="flex items-center gap-2 rounded-full bg-white px-4 py-2 font-bold shadow-sm"><Star className="h-5 w-5 fill-amber-400 text-amber-400" /> {score} / {maxRounds}</div>{onComplete && <Button variant="outline" onClick={onComplete}>Skip Game</Button>}</div>
    </div>
