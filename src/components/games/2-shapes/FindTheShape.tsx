      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6 w-full max-w-2xl mb-8">
        {choices.map((shape, idx) => {
          ...
          return (
            <button
              key={idx}
              className={`h-32 md:h-40 bg-[#ffe0a3] hover:bg-[#ffd17a] border-4 border-[#ffcd66] rounded-3xl flex items-center justify-center text-7xl md:text-8xl shadow-[0_6px_0_0_#e6aa32] transition-all active:translate-y-2 active:shadow-none ${
                gameState === 'feedback' && !isWinner ? 'opacity-50 grayscale' : ''
